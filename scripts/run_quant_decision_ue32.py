...
def evaluate_tasks(model, dataset_path: Path, device: torch.device, samples_per_task: int = 100) -> dict:
    ...
    allowed_tasks = ["binary_dir", "three_class_dir"]
...
def train_runner(mode: str, update_every: int, sft_dir: Path, device: torch.device, args: argparse.Namespace) -> dict:
    print(f"\n=================================================================")
    print(f"TRAINING MODE: {mode} (update_every={update_every})")
    print(f"=================================================================")
    
    out_dir = ROOT / "results_quant_decision_ue32" / mode
    out_dir.mkdir(parents=True, exist_ok=True)
    
    model = exp004b.make_model(device)
    
    # Load SFT datasets (passing tasks only)
    train_data = torch.from_numpy(np.fromfile(sft_dir / "train_passing.bin", dtype=np.uint8)).long()
    val_data = torch.from_numpy(np.fromfile(sft_dir / "val_passing.bin", dtype=np.uint8)).long()
    test_data = torch.from_numpy(np.fromfile(sft_dir / "test_passing.bin", dtype=np.uint8)).long()
    ...
    for step_idx in range(1, args.steps + 1):
        ...
        is_update_step = (step_idx % update_every == 0)
        
        if mode == "UE32" and not is_update_step:
            # Non-update step: forward + loss only (no autograd graph)
            with torch.no_grad():
                loss_v, _ = exp004b.cached_triton_forward_loss(model, None, x, y, amp)
            forward_loss_calls += 1
            lv = loss_v.item()
        else:
            # Update step: standard forward, backward, optimizer step
        ...
        if step_idx % args.eval_every == 0:
            ...
            acc_metrics = evaluate_tasks(model, sft_dir / "val_passing.jsonl", device, samples_per_task=100)
    ...
    # Test accuracy breakdown (100 samples per task)
    test_acc_metrics = evaluate_tasks(model, sft_dir / "test_passing.jsonl", device, samples_per_task=100)
...
def main():
    ...
    out_dir = ROOT / "results_quant_decision_ue32"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    # 1. Run UE1
    res_ue1 = train_runner("UE1", 1, sft_dir, device, args)
    
    # 2. Run UE32
    res_ue32 = train_runner("UE32", 32, sft_dir, device, args)
    ...
    combined = {
        ...
        "UE32": res_ue32,
        "comparison": {
            "speedup_ratio": res_ue32["train_input_tok_s"] / res_ue1["train_input_tok_s"],
            "accuracy_gap": res_ue32["test_accuracy_metrics"]["overall_accuracy"] - res_ue1["test_accuracy_metrics"]["overall_accuracy"]
        }
    }
    ...
    print("\nEXP010B Speed Sweep SFT Pipeline Completed successfully.")
