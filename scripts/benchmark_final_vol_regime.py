...
Verifies the EXP012 best_val_accuracy.pt checkpoint for:
...
REPORT_OUT      = ROOT / "docs" / "experiments" / "EXP013A_FINAL_BENCHMARK_AUDIT.md"
...
# Decision gate references from EXP012
REF_ACCURACY   = 0.7423
REF_MACRO_F1   = 0.742
REF_INVALID    = 0.00
...
def write_report(
    ...
    a("# EXP013A: Final Benchmark and Stability Audit")
    a("")
    ...
    a("")
    a(f"Reference from EXP012: accuracy ≈ {REF_ACCURACY:.2%}, macro F1 ≈ {REF_MACRO_F1:.3f}, "
    ...
    if abs(mean_acc - REF_ACCURACY) > MATERIAL_TOL:
        a(f"> ⚠️ **MATERIAL DIVERGENCE DETECTED**: mean accuracy {mean_acc:.4f} differs from "
          f"EXP012 reference {REF_ACCURACY:.4f} by more than ±{MATERIAL_TOL:.2%}.")
        ...
        a("> ✅ Accuracy is consistent with EXP012 reference within tolerance.")
    a("")
    ...
    if all_pass:
        ...
        a("The EXP012 checkpoint is approved as the final frozen artifact.")
    else:
...
def main():
    ...
    print("=" * 65)
    print("EXP013A: Final Benchmark and Stability Audit")
    print("=" * 65)
